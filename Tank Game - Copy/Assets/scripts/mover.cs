...
public class mover : MonoBehaviour
{
    ...
    void Update()
    {
        if(gameManager.isSingle && Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0){
            Move(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical"));
            Shoot();
        } 
        else if(Input.GetAxisRaw("Horizontal") != 0 || Input.GetAxisRaw("Vertical") != 0){
            Move(Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical"));
            gameManager.ProcessMove(gameManager.Players[0].UserID, Input.GetAxisRaw("Horizontal"), Input.GetAxisRaw("Vertical"), Time.time);
        }
